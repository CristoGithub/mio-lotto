...
<style>
    ...
    :root {
        ...
        --gold: #b45309;         /* Oro scuro/Ambra per contrasto */
...
def genera_pdf_mobile(ruota, ambi, terni, totale):
    ...
    pdf.ln(5)
    pdf.cell(190, 10, txt="AMBI SCELTI:", ln=True)
    for a in ambi: pdf.cell(190, 8, txt=f"- {a}", ln=True)
    pdf.ln(5)
    pdf.cell(190, 10, txt="TERNI SCELTI:", ln=True)
...
if file_arch:
    ...
    if not df_main.empty:
        ...
        with tab_anal:
            ...
            user_nums = st.text_input("✍️ Aggiungi numeri extra (es: 10-22)", "")
            extra = [int(x) for x in re.findall(r'\d+', user_nums) if 1 <= int(x) <= 90]
            pool = sorted(list(set(proposti + extra)))
            
            final_sel = st.multiselect("Conferma numeri per sviluppo:", options=pool, default=[n for n in pool if n in proposti or n in extra])
            ...
            if len(final_sel) >= 2:
                ...
                st.warning(f"Spesa Totale: {tot_spesa:.2f} €")
                ...
                st.download_button("📥 SCARICA SCHEDINA PDF", pdf_b, "schedina_lotto.pdf")
        ...
        with tab_bank:
            if os.path.exists(DB_FILE):
                ...
                st.metric("BILANCIO NETTO", f"{tot_vi - tot_sp:.2f} €", delta=f"{tot_vi - tot_sp:.2f} €")
                            ...
                            if r['Esito'] == "In gioco":
                                ...
                                for _, estr in check.iterrows():
                                    if target.issubset(set(estr['N'])):
                                        ...
                                        df_tot.at[idx, 'Esito'] = "VINTO"
                                        break
                        df_tot.to_csv(DB_FILE, index=False)
                        st.rerun()
                ...
                st.dataframe(df_tot.sort_values(by="Data", ascending=False), use_container_width=True)
            else:
                st.info("Nessuna giocata registrata.")

        # --- TAB CALCOLATRICE ---
        with tab_calc:
            st.markdown("### 🧮 Calcolo Vincite Netto")
            st.write("Calcolo basato sulla ritenuta fiscale dell'8%")
            
            tipo_v = st.radio("Sorte:", ["Ambo Secco", "Terno Secco"])
            puntata = st.number_input("Quanto vuoi puntare (€):", min_value=1.0, value=1.0, step=1.0)
            
            molt = MOLT_AMBO if tipo_v == "Ambo Secco" else MOLT_TERNO
            v_lorda = puntata * molt
            v_netta = v_lorda * (1 - TASSA_STATO)
            
            st.metric("VINCITA NETTA", f"{v_netta:.2f} €")
            st.write(f"Vincita Lorda: {v_lorda:.2f} €")
            st.write(f"Tasse (8%): -{(v_lorda * TASSA_STATO):.2f} €")
